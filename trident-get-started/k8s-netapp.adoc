...
https://www.netapp.com/google-cloud/netapp-volumes/["Google Cloud NetApp Volumes"^] ist ein Cloud-nativer Dateidienst, der NAS-Volumes über NFS mit All-Flash-Performance bereitstellt. Dieser Dienst ermöglicht die Ausführung beliebiger Workloads, einschließlich Legacy-Anwendungen, in der GCP Cloud. Er bietet einen vollständig verwalteten Dienst, der konstant hohe Leistung, sofortiges Klonen, Datenschutz und sicheren Zugriff auf Google Compute Engine (GCE) Instanzen bietet.
