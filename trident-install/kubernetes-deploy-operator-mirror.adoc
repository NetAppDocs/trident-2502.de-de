...
.Bevor Sie beginnen
Melden Sie sich am Linux-Host an und vergewissern Sie sich, dass er ein funktionierendes und link:../trident-get-started/requirements.html["Unterstützter Kubernetes-Cluster"] verwaltet und dass Sie über die erforderlichen Berechtigungen verfügen.
...
Aktualisieren Sie in `/deploy/operator.yaml`, `image: docker.io/netapp/trident-operator:25.02.0` um den Speicherort Ihrer Bildregistrierung anzuzeigen. Ihr link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident und CSI-Images"] kann sich in einer Registrierung oder in verschiedenen Registrierungen befinden, aber alle CSI-Bilder müssen sich in derselben Registrierung befinden. Beispiel:

* `image: <your-registry>/trident-operator:25.02.0` Wenn Ihre Bilder alle in einer Registrierung gespeichert sind.
* `image: <your-registry>/netapp/trident-operator:25.02.0` Wenn sich Ihr Trident-Image in einer anderen Registrierung als Ihre CSI-Images befindet.
