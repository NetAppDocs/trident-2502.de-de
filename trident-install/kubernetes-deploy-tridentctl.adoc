...
.Bevor Sie beginnen
Bevor Sie mit der Installation beginnen, melden Sie sich am Linux-Host an und vergewissern Sie sich, dass dieser ein funktionierendes link:../trident-get-started/requirements.html["Unterstützter Kubernetes-Cluster"] verwaltet und dass Sie über die erforderlichen Berechtigungen verfügen.
...
NOTE: Falls das Installationsprogramm nicht erfolgreich abgeschlossen wird oder  `trident-controller-<generated id>` (`trident-csi-<generated id>` in Versionen vor 23.01) keinen *Running*-Status hat, wurde die Plattform nicht installiert. Verwenden Sie  `-d` , um link:../troubleshooting.html#unsuccessful-trident-deployment-using-tridentctl["Aktivieren Sie den Debug-Modus"] das Problem zu beheben und zu analysieren.
...
[%collapsible]
====
Weitere Informationen zur Zwangstrennung sind unter link:kubernetes-customize-deploy.html["Anpassen der Trident Operator-Installation"] zu finden.
