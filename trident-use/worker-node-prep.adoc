...
.NFS Tools
link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#nfs-volumes["Installieren Sie die NFS Tools"] Wenn Sie: `ontap-nas`, , , `ontap-nas-economy` `ontap-nas-flexgroup` , `azure-netapp-files` `gcp-cvs` .

.ISCSI-Tools
link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#install-the-iscsi-tools["Installieren Sie die iSCSI-Tools"] Wenn Sie: `ontap-san`, , `ontap-san-economy` `solidfire-san` .

.NVMe-Tools
link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#nvmetcp-volumes["Installation der NVMe Tools"] Falls Sie das Protokoll Nonvolatile Memory Express (NVMe) over TCP (NVMe/TCP) verwenden `ontap-san`.
...
NOTE: Für Ubuntu 18.04 müssen Sie Zielports mit ermitteln `iscsiadm`, bevor der iSCSI-Daemon gestartet `open-iscsi` wird. Sie können den Dienst auch so ändern `iscsi`, dass er automatisch gestartet `iscsid` wird.
